...
public class BR_0731 {
	...
	public static void main(String[] args) {
		...
		int s = sc.nextInt();
		int e = sc.nextInt();
		int[] range = fnRearrange(s, e);
		
		while (range[1] < 2 || range[0] > 9) {
			System.out.println("INPUT_ERROR!");
			range = fnRearrange(s = sc.nextInt(), e = sc.nextInt());
		}
		sc.close();
		...
		for (int i = 1; i <= 9; i++) {
			int num = s;
			for (int j = 0; j <= range[1]-range[0]; j++) {
				table += num + " * " + i + " = " + String.format("%2d", i*num);
				if (num != e) table += String.format("%3s", " ");
				if (s == range[0]) num++;
				else num--;
			}
			table += "\n";
	...
	private static int[] fnRearrange(int from, int to) {
