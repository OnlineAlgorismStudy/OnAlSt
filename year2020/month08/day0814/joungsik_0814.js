...
stdin.on("data", function (key) {
  if (lastKey === " " && key === "0") {
    ...
    console.log("");
    console.log(
      `Multiples of 5 : ${nums.filter((num) => parseInt(num) % 5 === 0).length}`
    );
    console.log(
      `sum : ${nums
        .filter((num) => parseInt(num) % 5 === 0)
        .reduce((result, cost) => {
          return (result += isNaN(parseInt(cost)) ? 0 : parseInt(cost));
        }, 0)}`
    );
    console.log(
      `sum : ${(
        nums
          ...
          .reduce((result, cost) => {
            return (result += isNaN(parseInt(cost)) ? 0 : parseInt(cost));
          }, 0) / nums.filter((num) => parseInt(num) % 5 === 0).length
      ).toFixed(1)}`
    );
